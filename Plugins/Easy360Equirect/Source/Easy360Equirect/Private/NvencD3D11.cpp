...
bool FEasyNvencD3D11::Initialize(const FEasyNvencInit& Init)
{
    ...
    BitstreamFile.Reset(PF.OpenWrite(*OutPath));
    ...
    OpenParams.deviceType = static_cast<NV_ENC_DEVICE_TYPE>(
        (Init.DeviceType == EEasyNvencDeviceType::D3D12)
            ? NV_ENC_DEVICE_TYPE_DIRECTX12
            : NV_ENC_DEVICE_TYPE_DIRECTX11);
#else
    OpenParams.deviceType = static_cast<NV_ENC_DEVICE_TYPE>(NV_ENC_DEVICE_TYPE_DIRECTX);
#endif
...
bool FEasyNvencD3D11::EnsureRegistration(void* NativeResource)
{
    ...
    Register.resourceType = static_cast<NV_ENC_INPUT_RESOURCE_TYPE>(
        (CachedInit.DeviceType == EEasyNvencDeviceType::D3D12)
            ? NV_ENC_INPUT_RESOURCE_TYPE_DIRECTX12
            : NV_ENC_INPUT_RESOURCE_TYPE_DIRECTX);
#else
    Register.resourceType = static_cast<NV_ENC_INPUT_RESOURCE_TYPE>(NV_ENC_INPUT_RESOURCE_TYPE_DIRECTX);
#endif
...
bool FEasyNvencD3D11::DrainBitstream(bool bBlocking)
{
    ...
    {
        BitstreamFile->Write(static_cast<const uint8*>(Lock.bitstreamBufferPtr), Lock.bitstreamSizeInBytes);
    }
