...
AEqrCaptureActor::AEqrCaptureActor(){
  ...
  PreviewScreen=CreateDefaultSubobject<UStaticMeshComponent>(TEXT("PreviewScreen"));
  PreviewScreen->SetupAttachment(RootComponent);
  PreviewScreen->SetCollisionEnabled(ECollisionEnabled::NoCollision);
  PreviewScreen->SetCastShadow(false);
  PreviewScreen->SetGenerateOverlapEvents(false);
  PreviewScreen->bVisibleInReflectionCaptures=false;
  PreviewScreen->bVisibleInRayTracing=false;
  PreviewScreen->SetMobility(EComponentMobility::Movable);
  static ConstructorHelpers::FObjectFinder<UStaticMesh> PreviewPlaneMesh(TEXT("/Engine/BasicShapes/Plane.Plane"));
  if(PreviewPlaneMesh.Succeeded())
  {
    PreviewScreen->SetStaticMesh(PreviewPlaneMesh.Object);
  }
  if(!PreviewScreenMaterial)
  {
    static ConstructorHelpers::FObjectFinder<UMaterialInterface> DefaultPreviewMat(TEXT("/Engine/EngineMaterials/Widget3DPassThrough.Widget3DPassThrough"));
    if(DefaultPreviewMat.Succeeded())
    {
      PreviewScreenMaterial=DefaultPreviewMat.Object;
    }
  }
  ...
  UpdatePreviewScreenProperties();
  RefreshPreviewBinding();
}

void AEqrCaptureActor::BeginPlay(){ Super::BeginPlay(); FrameInterval=1.0/FMath::Max(1.0f,TargetFPS); UpdatePreviewScreenProperties(); RefreshPreviewBinding(); }

void AEqrCaptureActor::OnConstruction(const FTransform& Transform)
{
...
void AEqrCaptureActor::UpdatePreviewScreenProperties()
{
  if(!PreviewScreen)
  {
    return;
  ...
  const float SafeWidth = FMath::Max(1.f, PreviewScreenWidth);
  const float SafeHeight = FMath::Max(1.f, PreviewScreenHeight);

  PreviewScreen->SetRelativeLocation(PreviewScreenRelativeLocation);
  PreviewScreen->SetRelativeRotation(PreviewScreenRelativeRotation);
  PreviewScreen->SetRelativeScale3D(FVector(1.f, SafeWidth/100.f, SafeHeight/100.f));

  const bool bShouldBeVisible = bShowPreviewScreen;
  PreviewScreen->SetVisibility(bShouldBeVisible, true);
  PreviewScreen->SetHiddenInGame(!bShouldBeVisible);

  bPreviewVisibleLast = bShouldBeVisible;
  PreviewLastRelativeLocation = PreviewScreenRelativeLocation;
  PreviewLastRelativeRotation = PreviewScreenRelativeRotation;
  PreviewLastWidth = SafeWidth;
  PreviewLastHeight = SafeHeight;
}

void AEqrCaptureActor::RefreshPreviewBinding()
{
  if(!PreviewScreen)
  {
    return;
  ...
  if(!PreviewScreenMaterial)
  {
    if(PreviewMID)
    {
      PreviewScreen->SetMaterial(0, nullptr);
      PreviewMID = nullptr;
    }
    PreviewMaterialCached = nullptr;
    PreviewTextureCached = nullptr;
    return;
  }

  if(PreviewScreenMaterial != PreviewMaterialCached || !PreviewMID)
  {
    PreviewMID = UMaterialInstanceDynamic::Create(PreviewScreenMaterial, this);
    PreviewMaterialCached = PreviewScreenMaterial;
    PreviewTextureCached = nullptr;
    if(PreviewMID)
    {
      PreviewScreen->SetMaterial(0, PreviewMID);
    }
  }

  if(PreviewMID)
  {
    UTexture* TargetTexture = EquirectRT;
    if(PreviewTextureCached != TargetTexture)
    {
      PreviewMID->SetTextureParameterValue(PreviewTextureParameter, TargetTexture);
      PreviewTextureCached = TargetTexture;
    }
  }
}

#if WITH_EDITOR
void AEqrCaptureActor::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
...
void AEqrCaptureActor::EnsureTargets(){
  ...
  const bool bUse8BitTarget = bUseNvenc;
  EquirectRT->RenderTargetFormat = bUse8BitTarget ? ETextureRenderTargetFormat::RTF_RGBA8 : ETextureRenderTargetFormat::RTF_RGBA16f;
  bEquirectNeedsSRGB = bUse8BitTarget;
#else
  EquirectRT->RenderTargetFormat = ETextureRenderTargetFormat::RTF_RGBA16f;
  bEquirectNeedsSRGB = false;
#endif
...
void AEqrCaptureActor::Tick(float dt){
  ...
  if(PreviewScreen)
  {
    const bool bNeedsTransformRefresh =
      !PreviewLastRelativeLocation.Equals(PreviewScreenRelativeLocation, KINDA_SMALL_NUMBER) ||
      !PreviewLastRelativeRotation.Equals(PreviewScreenRelativeRotation, KINDA_SMALL_NUMBER) ||
      !FMath::IsNearlyEqual(PreviewLastWidth, PreviewScreenWidth) ||
      !FMath::IsNearlyEqual(PreviewLastHeight, PreviewScreenHeight) ||
      bPreviewVisibleLast != bShowPreviewScreen;

    if(bNeedsTransformRefresh)
    {
      UpdatePreviewScreenProperties();
    }

    if(PreviewScreenMaterial != PreviewMaterialCached || (!PreviewMID && PreviewScreenMaterial) || PreviewTextureCached != EquirectRT)
    {
      RefreshPreviewBinding();
    }
  }
...
void AEqrCaptureActor::ConvertEyeToRegion(FTextureRHIRef CubeRHI, const FIntRect& Region){
  ...
  const bool bApplySrgb = bEquirectNeedsSRGB;
  ENQUEUE_RENDER_COMMAND(Easy360_ConvertEye)([CubeRHI,Out,Region,Rot,bApplySrgb](FRHICommandListImmediate& RHICmdList){
    EqrRenderer::ConvertCubeToEquirect_RenderThread(RHICmdList, CubeRHI, Out, Region, Rot, bApplySrgb);
  });
}
